...
} from '../../../codebook-export/src/lib/codebook-export/codebook-export.component';
import {
  CodebookExportExecution,
  CodebookExportJobStatus,
  CodebookExportProvider
} from '../../../codebook-export/src/lib/codebook-export/codebook-export.provider';
import {
...
describe('CodebookExportComponent', () => {
  ...
  it('initializes inputs from dialog data', () => {
    const units: UnitSelectionItem[] = [{ unitId: 3, unitName: 'Unit 3.vocs', unitAlias: null }];
    const profiles: MissingsProfile[] = [{ id: 2, label: 'Custom' }];
    const dialogComponent = new CodebookExportComponent(undefined, {
      availableUnits: units,
      missingsProfiles: profiles,
      workspaceChanges: true,
      defaultContentOptions: { exportFormat: 'json' }
    });

    dialogComponent.ngOnInit();

    expect(dialogComponent.availableUnits).toBe(units);
    expect(dialogComponent.missingsProfiles).toEqual([{ id: 0, label: 'None' }, ...profiles]);
    expect(dialogComponent.selectedMissingsProfile).toBe(0);
    expect(dialogComponent.workspaceChanges).toBeTrue();
  ...
  it('clears the filter without requiring an input event', () => {
    component.availableUnits = [
      { unitId: 1, unitName: 'Alpha.vocs', unitAlias: null },
      { unitId: 2, unitName: 'Beta.vocs', unitAlias: null }
    ];
    fixture.detectChanges();

    component.filterValue = 'alpha';
    component.dataSource.filter = 'alpha';
