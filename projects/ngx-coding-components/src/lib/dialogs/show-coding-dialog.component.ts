...
import { CodingAsText, CodingToTextMode, ToTextFactory } from '@iqb/responses';
...
@Component({
  template: `
    <h1 mat-dialog-title>
      {{ varCoding.alias || varCoding.id
      }}{{ varCoding.label ? ' - ' + varCoding.label : '' }}
    </h1>
    ...
    <mat-dialog-content>
      ...
      <div class="info">
        <div class="fx-row-start-center">
          <div class="fx-flex-row-20">
            {{ 'manual-instruction.coding.source' | translate }}:
          </div>
        ...
        <div class="fx-row-start-center">
          ...
          <div class="fx-flex-fill">
            {{ varCodingText?.processing || 'keine' }}
          </div>
        </div>
        <div class="fx-row-start-center">
          <div class="fx-flex-row-20">
            {{ 'manual-instruction.coding.title' | translate }}:
          </div>
          <div class="fx-flex-fill">
            {{ varCodingText?.hasManualInstruction ? 'ja' : 'keine' }}
          </div>
        </div>
      </div>
      <h3>Codes</h3>
      @if (varCodingText && varCodingText.codes && varCodingText.codes.length >
      0) {
      <div class="fx-row-start-center" [style.font-weight]="'bold'">
        ...
        <div class="fx-flex-row-15" [style.text-align]="'center'">
          Instruktion
        </div>
      </div>
      ...
      <div class="fx-row-start-center code-row">
        ...
        <div class="fx-flex-fill">
          ...
          <p>
            Verknüpfung der Regelsätze:
            {{ codeText.ruleSetOperatorAnd ? 'UND' : 'ODER' }}
          </p>
          } @if (codeText.ruleSetDescriptions.length === 1) {
      ...
      } } @if (varCodingText && varCodingText.codes &&
      varCodingText.codes.length === 0) {
      <div>keine</div>
      }
    </mat-dialog-content>

    <mat-dialog-actions align="end">
      <button mat-raised-button color="primary" [mat-dialog-close]="mode">
...
export class ShowCodingDialogComponent {
  ...
  updateText(newValue: MatSlideToggleChange | boolean) {
    const booleanValue =
      typeof newValue === 'boolean' ? newValue : newValue.checked;
    ...
    this.varCodingText = {
      ...
      processing: ToTextFactory.processingAsText(
        this.varCoding.processing || [],
        this.varCoding.fragmenting
      ),
