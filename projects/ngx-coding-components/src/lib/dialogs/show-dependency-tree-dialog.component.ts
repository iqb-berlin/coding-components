...
@Component({
    ...
    <mat-dialog-content>
      ...
      <div class="fx-row-start-center fx-gap-15 var-row">
        <div class="fx-column-start-start">
          @if (vn.value.length > 0) { @for (vSource of vn.value; track vSource)
          {
  ...
  `,
  styles: ['.var-row:nth-child(even) {background-color: #f1f1f1;}'],
  standalone: true,
  imports: [
    MatDialogTitle,
    MatDialogContent,
...
export class ShowDependencyTreeDialogComponent {
  ...
  constructor(@Inject(MAT_DIALOG_DATA) public codingScheme: CodingScheme) {
    ...
    try {
      varNodes = CodingSchemeFactory.getVariableDependencyTree(
        ...
        varNodes
          ...
          .forEach(n => {
            newVarNodeRows.set(
              n.id,
              varNodes
                .filter(sn => n.sources.includes(sn.id))
                .map(cn => cn.id)
            );
          });
      }
