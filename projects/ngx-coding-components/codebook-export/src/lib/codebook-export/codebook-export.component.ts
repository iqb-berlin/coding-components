...
export interface CodebookExportDialogData {
  availableUnits?: UnitSelectionItem[];
  missingsProfiles?: MissingsProfile[];
  isLoading?: boolean;
  workspaceChanges?: boolean;
  defaultContentOptions?: Partial<CodeBookContentSetting>;
  provider?: CodebookExportProvider;
}
...
export class CodebookExportComponent implements OnInit, OnDestroy, OnChanges {
  ...
  constructor(
    ...
    @Optional() @Inject(MAT_DIALOG_DATA) private dialogData?: CodebookExportDialogData | null
  ) {}
  ...
  applyFilter(event: Event): void {
    const filterValue = (event.target as HTMLInputElement | null)?.value ?? this.filterValue;
  ...
  private applyDialogData(): void {
    ...
    if (this.dialogData.availableUnits) {
      this.availableUnits = this.dialogData.availableUnits;
    }
    if (this.dialogData.missingsProfiles) {
      this.missingsProfiles = this.ensureNoneProfile(this.dialogData.missingsProfiles);
      this.ensureSelectedMissingsProfile();
    }
    if (typeof this.dialogData.isLoading === 'boolean') {
      this.isLoading = this.dialogData.isLoading;
    }
    if (typeof this.dialogData.workspaceChanges === 'boolean') {
      this.workspaceChanges = this.dialogData.workspaceChanges;
    }
    if (this.dialogData.defaultContentOptions) {
      this.defaultContentOptions = {
        ...this.defaultContentOptions,
        ...this.dialogData.defaultContentOptions
      };
    }
