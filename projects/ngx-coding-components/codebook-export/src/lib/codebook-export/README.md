...
The component is available through the `@iqb/ngx-coding-components/codebook-export` entry point.
...
import {
  ...
  UnitSelectionItem,
  MissingsProfile
} from '@iqb/ngx-coding-components/codebook-export';
import { CodebookGenerator } from '@iqb/ngx-coding-components/codebook-generator';
...
} from '@iqb/ngx-coding-components/codebook-generator';
...
class MyComponent {
  async generateCodebook(config: CodebookExportConfig) {
    ...
    // Download the file
    this.downloadFile(blob, config.contentOptions.exportFormat);
  }
